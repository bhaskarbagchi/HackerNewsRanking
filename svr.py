...
for i in range(2, 10):
	...
	if clf.score(x_test, y_test) > last_score:
...
# print dataPoints
print clf.score(testPoints, expectedOutput)
# Mean Squared Error
print("Residual sum of squares: %.2f"
	% np.mean((clf.predict(testPoints) - expectedOutput) ** 2))

# Explained variance score: 1 is perfect prediction
print('Variance score: %.2f' % clf.score(testPoints, expectedOutput))


result = []
for x in dataPoints:
	result.append(clf.predict(x))
# print result
min_ = result[0]
max_ = result[0]
for x in result:
